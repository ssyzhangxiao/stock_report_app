...
import { SearchOutlined, DownloadOutlined, RiseOutlined, FallOutlined, FileSearchOutlined, LineChartOutlined } from '@ant-design/icons';
    ...
    <div style={styles.page}>
        ...
        <Row align="middle" justify="space-between" gutter={[16, 16]}>
          <Col xs={24} md={12}>
            <Space>
              ...
              <Segmented
                value={appMode}
                onChange={(v) => { setAppMode(v as 'realtime' | 'research'); setSelectedResearchDir(null); }}
                options={[
                  { label: <span><FileSearchOutlined /> 研究报告</span>, value: 'research' },
                  { label: <span><LineChartOutlined /> 实时数据</span>, value: 'realtime' },
                ]}
            ...
            <div style={styles.headerSub}>
              {appMode === 'research'
                ? '深度研究报告可视化 · 结构化数据呈现 · 多维度分析'
                : '多源数据驱动的智能分析平台 · 技能化架构 · 可视化报告'}
            </div>
          ...
          <Col xs={24} md={12}>
            {appMode === 'realtime' ? (
              ...
              <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, paddingTop: 8 }}>
                浏览 stock-research-agent 生成的深度研究报告，以可视化方式呈现
              </div>
            )}
          </Col>
        </Row>
      ...
      <div style={styles.contentArea}>
        ...
        {appMode === 'research' && (
          selectedResearchDir ? (
            <ResearchViewer
              directory={selectedResearchDir}
              onBack={() => setSelectedResearchDir(null)}
            />
          ) : (
